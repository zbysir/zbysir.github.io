...
jsx 转换之后 render 函数运行之后会得到一个类 dom 树，渲染它就相对简单更多了。
...
## 加载远端文件（主题）
Hollow 可以指定一个远程的 git 仓库地址作为主题，这得益于 [go-git](https://github.com/go-git/go-git) 库，go-git 底层使用的文件系统就是 [go-billy](https://github.com/go-git/go-billy)，这也是我选中 go-billy 作为 Hollow 的文件系统的原因。
